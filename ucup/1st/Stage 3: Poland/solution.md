...
## L. Line Replacements
트리가 주어졌을 때 조건을 만족하도록 경로들을 구성할 수 있을 조건은 다음과 같다.

Tram loop가 없는 모든 정점에 대하여,
* 인접한 간선의 가중치 합은 짝수이다.
* 인접한 간선의 가중치 중 가장 큰 값은 나머지 값의 합보다 작다.

이는 초기에 간단히 확인해줄 수 있고, 인접한 간선의 가중치 합과 가중치 집합을 관리하는 것으로 간선이 제거될 때마다 $O(\lg N)$에 확인해줄 수 있다.

또한 tram loop가 없는 한 정점에서 인접한 간선을 제거하는 것을 고려할 때, 앞서 제시한 조건 중 두 번째 조건 때문에 해가 존재한다면 가중치가 큰 간선부터 제거하는 해 역시 존재한다.
따라서 주어진 간선을 가중치 내림차순으로 정렬하여 다 제거시켜보고, 제거하는 중에 조건을 위배하는 정점이 등장하지 않는다면 해가 존재한다 판정하면 된다.

## M. Minor Evil
모든 사람 $i$에 대해서, 그 사람이 죽는 시간으로 가능한 가장 큰 값을 $dist[i]$라 하자.
만약 $dist[x]>t$이고 $a_t=x, b_t=y$였다면 $t$ 시점에서 $x$는 살아있으며 $y$를 죽일 수 있다. 
따라서 $dist[y] \ge t$이다.

리스트에 있는 모든 사람 $i$에 대해 $dist[i]=0$, 그렇지 않은 사람에 대해 $dist[i]=\infty$로 초기화해두고 위와 같은 $x, y$가 존재할 때마다 $dist[y]:=\max(dist[y], t)$로 갱신해준다면 올바른 $dist$ 배열을 구할 수 있다.
위 과정은 dijkstra 알고리즘을 통해 빠르게 할 수 있다.
